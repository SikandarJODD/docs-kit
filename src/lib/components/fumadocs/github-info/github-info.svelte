<script lang="ts">
  ...
  let stars = $state(0);
  let humanizedStars = $derived(humanizeNumber(stars));
  onMount(async () => {
    try {
      const data = await getRepoStarsAndForks(owner, repo, token, baseUrl);
      stars = data.stars;
    } catch (error) {
...
<a
  ...
  {...props}
  class={cn(
    "flex flex-col gap-2 p-2 rounded-lg text-sm text-fd-foreground/80 transition-colors lg:flex-row lg:items-center hover:text-fd-accent-foreground hover:bg-fd-accent",
    props.class
  )}
>
  <p class="flex items-center gap-2 truncate">
    <svg fill="currentColor" viewBox="0 0 24 24" class="size-3.5">
      <title>GitHub</title>
      <path
  ...
  <p class="flex text-xs items-center gap-1 text-fd-muted-foreground">
    <Star class="size-3" />
    {humanizedStars}
  </p>
</a>
