...
{#if isBlock(typeof className === "string" ? className : undefined, restProps["data-theme"])}
  <code
    {...restProps}
    class={cn("block text-sm leading-relaxed whitespace-pre mono", className)}
  >
    {@render children?.()}
  </code>
{:else}
  <div
    class="relative inline-block w-fit rounded-lg border border-border bg-card px-1.5 py-0.5 text-xs font-medium text-foreground shadow-sm mono"
  >
